...
export default function App() {
  ...
  // Force dark mode for the technological look initially, but allow toggle
  useEffect(() => {
    if (isDarkMode) {
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-cyan-400">
        <motion.div
          animate={{ rotate: 360 }}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4 relative overflow-hidden">
        ...
        <motion.div 
          ...
          className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 rounded-3xl shadow-2xl max-w-md w-full text-center relative z-10"
        >
          <motion.div 
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Tool Manager <span className="text-cyan-400">OS</span></h1>
          <p className="text-slate-400 dark:text-slate-500 dark:text-slate-400 mb-8 text-sm">Sistema avançado de gestão e padronização de ferramentaria.</p>
          ...
          <button
            ...
            disabled={isLoggingIn}
            className="w-full flex items-center justify-center gap-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium py-3 px-4 rounded-xl transition-all border border-slate-300 dark:border-slate-700"
          >
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans text-slate-800 dark:text-slate-200 selection:bg-cyan-500/30 relative overflow-hidden">
      ...
      {/* Mobile Header */}
      <div className="md:hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 p-4 flex justify-between items-center sticky top-0 z-30">
        <div className="font-bold text-lg flex items-center gap-3 text-slate-900 dark:text-white">
        ...
        <button onClick={() => setIsMobileMenuOpen(true)} className="text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white">
      ...
      <motion.div 
        className={`
          fixed inset-y-0 left-0 z-50 w-64 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 flex-shrink-0 flex flex-col h-screen
      ...
      >
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 relative flex-shrink-0">
              ...
              <div className="hidden w-full h-full bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl items-center justify-center shadow-[0_0_15px_rgba(6,182,212,0.4)]">
                <Wrench className="w-5 h-5 text-slate-900 dark:text-white" />
              </div>
            </div>
            <span className="font-bold text-lg tracking-tight text-slate-900 dark:text-white">Tool Manager</span>
          </div>
          <button className="md:hidden text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white" onClick={() => setIsMobileMenuOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-left relative group ${
                  isActive 
                    ...
                    : 'text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:bg-slate-100/50 dark:bg-slate-800/50 hover:text-slate-800 dark:text-slate-200'
                }`}
              >
                {isActive && (
                  <motion.div 
        ...
        <div className="p-4 border-t border-slate-200/50 dark:border-slate-800/50 bg-white/30 dark:bg-slate-900/30">
          <div className="flex items-center gap-3 mb-3">
            {user.photoURL ? (
              <Image src={user.photoURL} alt="Profile" width={36} height={36} className="rounded-full border border-slate-300 dark:border-slate-700" referrerPolicy="no-referrer" />
            ) : (
              <div className="w-9 h-9 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-full flex items-center justify-center">
                <Users className="w-4 h-4 text-slate-400 dark:text-slate-500 dark:text-slate-400" />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{user.displayName || 'Operador'}</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 truncate">{user.email}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:bg-red-500/10 hover:text-red-400 border border-transparent hover:border-red-500/20 transition-all text-sm font-medium"
          >
