    ...
    >
      ...
      <motion.div variants={itemVariants} className="bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-800 overflow-hidden">
        <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/80">
          <h2 className="text-lg font-semibold text-white">Catálogo de Ferramentas</h2>
          <div className="relative w-full sm:w-auto">
            ...
            <input 
              ...
              className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-slate-950/50 border border-slate-700 rounded-xl text-sm text-slate-200 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 outline-none transition-all"
            />
          </div>
        </div>
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-950/50 text-slate-400 text-xs uppercase tracking-wider font-mono border-b border-slate-800">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filteredTools.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500 font-mono text-sm">Nenhuma ferramenta encontrada.</td>
                </tr>
              ) : (
                filteredTools.map((tool, idx) => (
                  ...
                  >
                    <td className="p-4 font-medium text-slate-200">{tool.brand}</td>
                    <td className="p-4 text-slate-300">{tool.name}</td>
                    <td className="p-4 text-slate-400 capitalize">
                      <span className="px-2.5 py-1 bg-slate-800 rounded-md text-xs border border-slate-700">{tool.category}</span>
                    </td>
