  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-white tracking-tight">Colaboradores e Departamentos</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <ul className="space-y-2">
                {sortedDepartments.map(dept => (
                  <motion.li 
                    ...
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-800/50 border border-transparent transition-colors"
                  >
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-800 flex flex-col h-[500px] relative overflow-hidden">
          ...
          <div className="p-5 border-b border-slate-800 bg-slate-900/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5 text-emerald-400" />
              <h2 className="text-lg font-semibold text-white">{editingEmpId ? 'Editar Colaborador' : 'Colaboradores'}</h2>
            </div>
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <input
                ...
                className="flex-1 sm:flex-none text-sm p-2 bg-slate-950/50 border border-slate-700 rounded-xl text-slate-200 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all sm:w-48"
              />
              <button
                ...
                {employees
                  ...
                  return (
                    <motion.div 
                      ...
                      className="p-4 border border-slate-800 rounded-xl flex items-start justify-between bg-slate-950/30 hover:border-slate-700 hover:bg-slate-800/30 transition-all group"
                    >
                      <div className="flex-1 pr-2">
                        <p className="font-medium text-sm text-slate-200">{emp.name}</p>
                        <p className="text-[10px] font-mono text-slate-500 mt-1 uppercase tracking-wider">Matrícula: <span className="text-slate-400">{emp.employeeId}</span></p>
