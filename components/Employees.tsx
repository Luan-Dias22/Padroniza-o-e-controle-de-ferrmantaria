  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold font-sans tracking-tight text-white tracking-tight">Colaboradores e Departamentos</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="bg-[#0a0a0a] rounded-xl shadow-xl border border-white/105 overflow-hidden flex flex-col h-[500px] relative">
          ...
          <div className="p-5 border-b border-white/5 bg-zinc-900/80 flex items-center gap-3">
            <Building2 className="w-5 h-5 text-emerald-400" />
            <h2 className="text-lg font-semibold font-sans tracking-tight text-white">Departamentos</h2>
          </div>
          {!isGuest && (
            <div className="p-4 border-b border-white/5 bg-transparent/30">
              <form onSubmit={handleAddDepartment} className="flex gap-2">
                <input 
                  ...
                  className="flex-1 p-2.5 bg-zinc-900/30 border border-white/105 rounded-xl text-sm text-zinc-200 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none transition-all"
                />
          ...
          <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
            {sortedDepartments.length === 0 ? (
              <p className="text-center text-zinc-500 p-4 text-sm font-mono">Nenhum departamento criado.</p>
            ) : (
              <ul className="space-y-2">
                {sortedDepartments.map(dept => (
                  <motion.li 
                    ...
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-zinc-900/50 border border-transparent transition-colors"
                  >
                    {editingDeptId === dept.id ? (
                      <div className="flex flex-col gap-3 w-full" onClick={e => e.stopPropagation()}>
                        <input 
                          ...
                          className="w-full p-2 bg-transparent border border-white/105 rounded-lg text-sm text-zinc-200 outline-none focus:border-emerald-500"
                          placeholder="Nome do departamento"
                        />
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider whitespace-nowrap">Novatos previstos:</span>
                            <input 
                              type="number"
                              min="0"
                              value={editDeptNewcomers} 
                              onChange={e => setEditDeptNewcomers(parseInt(e.target.value) || 0)}
                              className="w-16 p-1.5 bg-transparent border border-white/105 rounded-lg text-sm text-zinc-200 outline-none focus:border-emerald-500"
                            />
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider whitespace-nowrap">Funcionários Nec.:</span>
                            <input 
                              type="number"
                              min="0"
                      ...
                      <div className="flex flex-col w-full">
                        <div className="flex items-center justify-between w-full">
                          <span className="font-medium text-sm text-zinc-200">{dept.name}</span>
                          <div className="flex gap-1">
                            {!isGuest && (
                              <>
                                ...
                                <button onClick={() => handleDeleteDepartment(dept.id)} className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-[#0a0a0a] rounded-xl shadow-xl border border-white/105 flex flex-col h-[500px] relative overflow-hidden">
          ...
          <div className="p-5 border-b border-white/5 bg-zinc-900/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Users className="w-5 h-5 text-emerald-400" />
              <h2 className="text-lg font-semibold font-sans tracking-tight text-white">{editingEmpId ? 'Editar Colaborador' : 'Colaboradores'}</h2>
            </div>
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <input
                ...
                className="flex-1 sm:flex-none text-sm p-2 bg-zinc-900/30 border border-white/105 rounded-xl text-zinc-200 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all sm:w-48"
              />
              <button
          ...
          {!isGuest && (
            <div className="p-5 border-b border-white/5 bg-transparent/30">
              <form onSubmit={handleSubmitEmployee} className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <input 
                  ...
                  className="p-2.5 bg-zinc-900 border border-white/105 rounded-xl text-sm text-zinc-200 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none transition-all"
                />
                <input 
                  type="text" 
                  value={newEmpName} 
                  ...
                  className="p-2.5 bg-zinc-900 border border-white/105 rounded-xl text-sm text-zinc-200 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none transition-all sm:col-span-2"
                />
                <select
                  value={newEmpDept}
                  onChange={e => setNewEmpDept(e.target.value)}
                  className="p-2.5 bg-zinc-900 border border-white/105 rounded-xl text-sm text-zinc-300 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 outline-none transition-all appearance-none"
                >
                ...
                <div className="sm:col-span-4 flex justify-end gap-3 mt-2">
                  ...
                  <button type="submit" className="px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl hover:from-emerald-500 hover:to-teal-500 flex items-center justify-center gap-2 shadow-sm transition-all">
                ...
                {employees
                  ...
                  return (
                    <motion.div 
                      ...
                      className="p-4 border border-white/105 rounded-xl flex items-start justify-between bg-transparent/30 hover:border-white/5 hover:bg-zinc-900/30 transition-all group"
                    >
                      <div className="flex-1 pr-2">
                        <p className="font-medium text-sm text-zinc-200">{emp.name}</p>
                        <p className="text-[10px] font-mono text-zinc-500 mt-1 uppercase tracking-wider">Matrícula: <span className="text-zinc-400">{emp.employeeId}</span></p>
                      ...
                      <div className="flex gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!isGuest && (
                          <>
                            <button onClick={() => handleEditEmployee(emp)} className="p-1.5 text-zinc-500 hover:text-cyan-400 hover:bg-cyan-500/10 rounded-lg transition-colors">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteEmployee(emp.id)} className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </>
                        )}
