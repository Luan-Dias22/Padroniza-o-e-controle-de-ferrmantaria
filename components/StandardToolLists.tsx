  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Kits de Ferramentas Padrão</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[500px] relative">
          ...
          <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Kits Padrão</h2>
          </div>
          {!isGuest && (
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/30">
              <form onSubmit={handleAddKit} className="flex gap-2">
                <input 
                  ...
                  className="flex-1 p-2.5 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 outline-none transition-all"
                />
          ...
          <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
            {standardLists.length === 0 ? (
              <p className="text-center text-slate-400 dark:text-slate-500 p-4 text-sm font-mono">Nenhum kit criado.</p>
            ) : (
                  ...
                  <motion.li 
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${
                      selectedKitId === kit.id 
                        ...
                        : 'hover:bg-slate-100/50 dark:bg-slate-800/50 border border-transparent'
                    }`}
                    ...
                    {editingKitId === kit.id ? (
                      <div className="flex items-center gap-2 w-full" onClick={e => e.stopPropagation()}>
                        <input 
                          ...
                          className="flex-1 p-1.5 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-purple-500"
                        />
                      ...
                      <>
                        <span className={`font-medium text-sm ${selectedKitId === kit.id ? 'text-purple-400' : 'text-slate-700 dark:text-slate-300'} flex items-center gap-2`}>
                          {kit.name}
                          {kit.isLocked && <Lock className="w-3 h-3 text-slate-400 dark:text-slate-500" />}
                        </span>
                        <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                          {!isGuest && (
                            <button 
                              onClick={() => handleToggleLock(kit.id)} 
                              className={`p-1.5 rounded-lg transition-colors ${kit.isLocked ? 'text-amber-400 hover:bg-amber-500/10' : 'text-slate-400 dark:text-slate-500 hover:text-purple-400 hover:bg-purple-500/10'}`}
                          ...
                          {!kit.isLocked && !isGuest && (
                            <>
                              <button onClick={() => { setEditingKitId(kit.id); setEditKitName(kit.name); }} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-cyan-400 hover:bg-cyan-500/10 rounded-lg transition-colors">
                                <Edit2 className="w-4 h-4" />
                              </button>
                              <button onClick={() => handleDeleteKit(kit.id)} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col h-[500px] relative overflow-hidden">
          ...
          <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
                ...
                <>
                  <div className="relative flex-1 sm:flex-none">
                    ...
                    <input
                      ...
                      className="w-full sm:w-48 pl-9 pr-3 py-2 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all"
                    />
                  </div>
                  <button
                  ...
                  return (
                    <div 
                      ...
                      className={`p-3 border rounded-xl flex items-start gap-3 transition-all ${
                        isSelected ? 'border-purple-500/50 bg-purple-500/5' : 'border-slate-200 dark:border-slate-800 hover:border-slate-600 hover:bg-slate-100/30 dark:bg-slate-800/30'
                      }`}
                    >
                      <div 
                        className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${
                          ...
                          isSelected ? 'bg-purple-500 border-purple-500' : 'border-slate-600 bg-white dark:bg-slate-900'
                        }`} 
                      ...
                      <div 
                        ...
                        onClick={() => !currentKit?.isLocked && !isGuest && toggleToolInKit(tool.id)}
                      >
                        <p className={`font-medium text-sm ${isSelected ? 'text-purple-300' : 'text-slate-700 dark:text-slate-300'}`}>{tool.name}</p>
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{tool.brand} • {tool.category}</p>
                      </div>
                      {isSelected && (
                        <div className="flex items-center gap-2">
                          <label className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase">Qtd:</label>
                          <input 
                            type="number" 
                            min="1"
                            ...
                            className="w-16 p-1.5 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg text-sm text-slate-800 dark:text-slate-200 outline-none focus:ring-1 focus:ring-purple-500 disabled:opacity-50"
                          />
                        </div>
                      )}
                    </div>
      ...
      {/* Departments Linkage */}
      <motion.div variants={itemVariants} className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden relative">
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-cyan-500/10 border border-cyan-500/30 rounded-lg flex items-center justify-center">
              <LinkIcon className="w-4 h-4 text-cyan-400" />
            </div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Departamentos e Kits</h2>
          </div>
        </div>
        <div className="p-5">
          {departments.length === 0 ? (
            <p className="text-slate-400 dark:text-slate-500 text-sm font-mono">Nenhum departamento registrado. Crie departamentos na aba Colaboradores.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...departments].sort((a, b) => sortByName(a.name, b.name)).map((dept, idx) => (
                <motion.div 
                  ...
                  className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 bg-slate-50/30 dark:bg-slate-950/30 flex flex-col gap-4 hover:border-slate-300 dark:border-slate-700 transition-colors"
                >
                  ...
                  <div className="space-y-3">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-mono text-slate-400 dark:text-slate-500">Kit Individual</label>
                      <select
                        value={dept.standardListId || ''}
                        disabled={isGuest}
                        onChange={e => handleLinkKitToDepartment(dept.id, e.target.value)}
                        className="w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 outline-none transition-all appearance-none disabled:opacity-50"
                      >
