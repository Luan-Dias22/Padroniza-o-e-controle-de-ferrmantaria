    ...
    >
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Atribuições de Funcionários</h1>
        </div>
        {!isAssigning && !isGuest && (
          <button 
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-slate-900 dark:text-white rounded-xl hover:from-blue-500 hover:to-indigo-500 flex items-center gap-2 shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all"
          >
      ...
      <AnimatePresence mode="wait">
        {isAssigning && (
          <motion.div 
            ...
            className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6 relative"
          >
            ...
            <div className="flex justify-between items-center mb-6 border-b border-slate-200 dark:border-slate-800 pb-4">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{editingAssignmentId ? 'Editar Atribuição' : 'Criar Atribuição'}</h2>
              <button onClick={() => setIsAssigning(false)} className="p-2 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:bg-slate-800 rounded-full transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Selecionar Funcionário</label>
                  <div className="flex gap-2 mb-3">
                    <input
                      ...
                      className="flex-1 p-2.5 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all"
                    />
                    <button
                  ...
                  <select 
                    ...
                    className="w-full p-2.5 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all appearance-none"
                  >
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className="text-sm p-2 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all w-full sm:w-40"
                    />
                    <select 
                      value={assignmentFilter}
                      onChange={e => setAssignmentFilter(e.target.value as any)}
                      className="text-sm p-2 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all appearance-none"
                    >
                ...
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-3 h-64 overflow-y-auto bg-slate-50/30 dark:bg-slate-950/30 custom-scrollbar">
                  {!selectedEmployeeId ? (
                    <div className="h-full flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm text-center font-mono">
                      Selecione um funcionário primeiro para carregar as ferramentas padrão.
                    </div>
                  ) : tools.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm font-mono">Nenhuma ferramenta disponível.</div>
                  ) : (
                        ...
                        .map(tool => {
                          ...
                          <motion.div 
                            ...
                            className={`p-3 border rounded-xl flex items-center gap-3 transition-all ${
                              isSelected ? 'bg-blue-900/20 border-blue-500/50 shadow-[0_0_10px_rgba(59,130,246,0.1)]' : 'bg-white/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:border-slate-700 hover:bg-slate-100/50 dark:bg-slate-800/50'
                            }`}
                          >
                            <div 
                              className={`w-5 h-5 rounded-md border flex items-center justify-center flex-shrink-0 cursor-pointer transition-colors ${
                                isSelected ? 'bg-blue-500 border-blue-500' : 'border-slate-600 bg-slate-50 dark:bg-slate-950'
                              }`}
            ...
            <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-800 pt-5">
              <div className="flex items-center gap-3">
                  ...
                  <div 
                    className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${
                      shouldDeductFromStock 
                        ...
                        : 'border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 group-hover:border-slate-600'
                    }`}
                    onClick={() => setShouldDeductFromStock(!shouldDeductFromStock)}
                  >
                    {shouldDeductFromStock && <Check className="w-3.5 h-3.5 text-slate-900 dark:text-white" />}
                  </div>
                  <span className="text-sm text-slate-700 dark:text-slate-300 group-hover:text-slate-800 dark:text-slate-200 transition-colors select-none">
              ...
              <div className="flex gap-3 w-full sm:w-auto items-center">
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-none px-5 py-2.5 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-100 dark:bg-slate-800 transition-colors"
                >
                  Cancelar
                </button>
                <button 
                  ...
                  className="flex-1 sm:flex-none px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-slate-900 dark:text-white rounded-xl hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all"
                >
                  Salvar Atribuição
                </button>
              </div>
      ...
      {!isAssigning && (
        <motion.div variants={itemVariants} className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Atribuições Atuais</h2>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              <input
                ...
                className="text-sm p-2 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all w-full sm:w-64"
              />
              <select
                value={assignmentDepartmentFilter}
                onChange={(e) => setAssignmentDepartmentFilter(e.target.value)}
                className="text-sm p-2 bg-slate-50/50 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all appearance-none w-full sm:w-auto"
              >
              ...
              <div className="flex items-center gap-2">
                <label className="text-xs font-mono text-slate-400 dark:text-slate-500 flex items-center gap-1 uppercase tracking-wider">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filteredAssignments.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400 dark:text-slate-500 font-mono text-sm">Nenhuma atribuição encontrada.</td>
                  </tr>
                ) : (
                  filteredAssignments.map((assignment, idx) => {
                    ...
                    return (
                      <motion.tr 
                        ...
                        key={assignment.id} 
                        className="border-b border-slate-200/50 dark:border-slate-800/50 hover:bg-slate-100/30 dark:bg-slate-800/30 transition-colors group"
                      >
                        <td className="p-4">
                          <button 
                          ...
                          >
                            <p className="font-medium text-slate-800 dark:text-slate-200 group-hover:text-blue-400 transition-colors">{emp?.name || 'Desconhecido'}</p>
                            <p className="text-[10px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-1">{emp?.employeeId}</p>
                          </button>
                        </td>
                        <td className="p-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 text-sm">{dept?.name || 'Desconhecido'}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 text-sm font-mono">{date}</td>
                        <td className="p-4 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleViewAssignment(assignment)} className="p-2 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-200 dark:bg-slate-700 rounded-lg transition-colors" title="Ver Detalhes">
            ...
            >
              ...
              >
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white/80 dark:bg-slate-900/80">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Detalhes da Atribuição</h2>
                    <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400 font-mono mt-1">
                      {employees.find(e => e.id === viewingAssignment.employeeId)?.name} • {new Date(viewingAssignment.dateAssigned).toLocaleDateString()}
                    </p>
                  </div>
                  <button 
                    onClick={() => setViewingAssignment(null)}
                    className="p-2 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:bg-slate-800 rounded-full transition-colors"
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <div className="grid grid-cols-2 gap-6 mb-8">
                    <div className="bg-slate-50/50 dark:bg-slate-950/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                      <p className="text-[10px] font-mono font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Colaborador</p>
                      <p className="font-bold text-slate-900 dark:text-white text-lg">{employees.find(e => e.id === viewingAssignment.employeeId)?.name}</p>
                      <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400 font-mono mt-1">ID: {employees.find(e => e.id === viewingAssignment.employeeId)?.employeeId}</p>
                    </div>
                    <div className="bg-slate-50/50 dark:bg-slate-950/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                      <p className="text-[10px] font-mono font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Departamento</p>
                      <p className="font-bold text-slate-900 dark:text-white text-lg">{departments.find(d => d.id === viewingAssignment.departmentId)?.name}</p>
                      <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400 font-mono mt-1">Data: {new Date(viewingAssignment.dateAssigned).toLocaleDateString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50/30 dark:bg-slate-950/30">
                        ...
                        {(viewingAssignment.assignedTools || []).map((at, idx) => {
                          ...
                          return (
                            <tr key={idx} className="border-b border-slate-200/50 dark:border-slate-800/50 last:border-0 hover:bg-slate-100/30 dark:bg-slate-800/30 transition-colors">
                              <td className="p-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 text-sm">{tool?.brand}</td>
                              <td className="p-4">
                ...
                <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 flex justify-end gap-3">
                  <button 
                    onClick={() => setViewingAssignment(null)}
                  ...
                  <button 
                    ...
                    className="px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-slate-900 dark:text-white rounded-xl hover:from-emerald-500 hover:to-teal-500 transition-all flex items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.3)]"
                  >
                    <Download className="w-4 h-4" />
                    Exportar PDF
                  </button>
